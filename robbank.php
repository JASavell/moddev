...
function robbank_getmoduleinfo(){
	$info = array(
		...
		"requires"=>array(
			"Specialty - Rogue "=>"1.0|`@Miskatonic`2, available on Dragonprime.net",
			),
		"settings"=>array(
			"Bank robbing Settings,title",
				...
				"onlyrogue"=>"Only rogue specialty?, bool|1",
...
/*		case "newday":
		...
		$specialty = mysql_query("SELECT specialty FROM accounts");
		$result = mysql_fetch_array($specialty);
...
if ($op == "count") {
	...
	$totalgold=0;	
			...
			$sql = "SELECT acctid,goldinbank,specialty FROM " . db_prefix("accounts") . " where goldinbank>=$mingold and dragonkills>=$mindk and level>=$minlvl and acctid!=$accntid and OR=$specialty";
			$result = db_query($sql);
